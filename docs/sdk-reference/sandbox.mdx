...
| `previewAccess` / `preview_access`          | `"public" \| "private"`  | Explicit preview policy. Both require published ports; `private` also requires a per-port credential. Omit only for legacy compatibility. See [Preview URLs](/sandbox/preview-urls). |
...
Patch `metadata`, `network`, lifecycle windows, and/or the preview policy on a sandbox.
...
publication and migration flow.
...
| `listPreviewPorts()`                  | `list_preview_ports()`                                | Return the effective policy and published ports; tokens are never listed. |
| `publishPreviewPort(port)`            | `publish_preview_port(port)`                          | Idempotently publish a port.                                              |
...
| `previewAccess` / `preview_access` | `"legacy_public" \| "public" \| "private"` | Effective preview policy at construction time.                                       |
