...
description: "Keep control of where Cursor Cloud Agents execute - the image, the network, the credentials - without running a worker fleet. One Superserve sandbox per request."
icon: "/logos/integrations/cursor.svg"
---

[Cursor Cloud Agents](https://cursor.com/docs/cloud-agent) are Cursor's background agents: you hand one a task from Cursor, Slack, GitHub, or Linear, and it works the repo and opens a PR. By default the agent's tool calls run on VMs Cursor manages. [Self-Hosted Machines](https://cursor.com/docs/cloud-agent/self-hosted) moves just that part - the checkout, file edits, and shell commands - onto a worker you control. Cursor keeps the agent loop, model calls, and the Cloud Agent UI.

Teams reach for Self-Hosted Machines when they need to own the execution environment: a private package registry, services only reachable from their network, a custom image, or a requirement to know exactly which machine an agent's commands ran on and what it reached. The usual price is a worker fleet to size, patch, and keep on call.

This guide runs each worker in a Superserve sandbox instead. You get the control that Self-Hosted Machines is for, on infrastructure that exists only for the length of a request.

## Why Superserve

- **A fresh sandbox per request.** Each Cloud Agent runs in its own isolated Superserve sandbox, created when the request is claimed and deleted when the worker exits. No shared hosts, no state left behind for the next agent to find.
- **Your image, your toolchains.** Workers boot from a [template](/templates/overview) you build: runtimes, package mirrors, internal CA certificates, whatever your repos need. Nothing is installed at claim time.
- **Egress you define, and a record of what was reached.** Set per-sandbox [network rules](/sandbox/networking) so a worker can reach Cursor, your git host, and nothing else. The [network log](/sandbox/network-log) records every connection a worker made, allowed or blocked.
- **Credentials stay where they belong.** Your Superserve key never enters a sandbox. The worker's Cursor key is scoped to its process. Repo access uses per-run tokens Cursor mints, so there is no long-lived git credential anywhere in the setup.
- **Pause between turns.** A follow-up an hour later should not start from an empty checkout. `pause()` checkpoints the whole sandbox at zero compute cost; `resume()` brings it back in under 50ms with the workspace intact.
- **Nothing to keep warm.** Capacity scales from zero. No machines run before a request arrives and none linger after.
...
<Tip>
  Add the runtimes, package caches, and internal CA certificates your repos need
  to the build steps. The snapshot captures the full filesystem, so workers
  inherit everything with no install cost at claim time.
</Tip>
...
<Warning>
  ...
  sandbox. `CURSOR_API_KEY` is passed on the launch command rather than as a
  sandbox-wide env var, so only the worker's process tree can read it.
</Warning>
...
To restrict egress, set `CURSOR_WORKER_ALLOW_OUT` to a comma-separated list of hosts and CIDRs. The spawn hook then creates sandboxes that deny everything else. See [Network rules](/sandbox/networking) for the format and the [network log](/sandbox/network-log) for every connection a worker attempted, allowed or blocked.
...
<Note>
  Git access comes from tokens Cursor mints per run. There is no personal access
  token in `.env`, in the template, or in the sandbox. Use a dedicated service
  account per pool so a compromised worker can only claim that pool's work.
</Note>
...
| `CURSOR_POOL`                        | set by controller | Pool name. The spawn hook receives it per claim; the monitor reads it from `.env` to scope its sweeps and wake-ups to that pool |
...
<CardGroup cols={2}>
  <Card
    ...
    icon="github"
    href="https://github.com/superserve-ai/superserve/tree/main/guides/managed-agents/cursor-cloud-agents"
  >
    Template builder, spawn hook, and monitor in TypeScript and Python.
  </Card>
  <Card
    title="Cursor Team Pools"
    icon="book"
