...
That control is the point. You decide which image the agent runs on, what it can reach, and what happens to the workspace when it's done. The catch, and Cursor's own docs are candid about it, is that a Team Pool means managing a fleet of machines: keeping enough workers online, patching images, resetting machines between runs, scaling for bursts, and cleaning up after every session.
