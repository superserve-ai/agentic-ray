...
**Ports.** New MCP sandboxes default to the strict `public` preview policy:
only explicitly published ports are reachable. Pass
`preview_access: "private"` to `sandbox_create` (or `sandbox_update`) when a
credential should be required. Start the server with `sandbox_exec`, then call
`sandbox_preview_url`; the tool idempotently publishes that one port and
returns either a clean public URL or an expiring signed URL for a private
sandbox. Private links default to one hour; set `expires_in_seconds` to a value
from 1 through 604800 seconds. See [Preview URLs](/sandbox/preview-urls).
