...
4. Writes `.github/workflows/loop-pr-loop.yml` — the only file added to your repo — then commits and pushes it for you.

Push a commit to any PR and it is reviewed within seconds.

<Note>
  If the installer can't push the workflow (no write access, detached HEAD, a
  blocking pre-commit hook), it prints the exact `git add` / `commit` / `push`
...
<CardGroup cols={2}>
  <Card
    ...
    href="https://github.com/superserve-ai/superserve/tree/main/examples/loops/pr-loop"
  >
