...
4. Writes `.github/workflows/loop-pr-loop.yml` — the only file added to your repo.
...
<Note>
  ...
  pr-loop …`) — no loop source is vendored into your repo. `@stable` is a
...
In `.github/workflows/loop-pr-loop.yml`, add the token-mint step and point the loop at it:
